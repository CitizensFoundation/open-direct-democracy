...
require '/home/robert/sites/open-direct-democracy/master/current/config/boot'
require "#{RAILS_ROOT}/config/environment"

require '/home/robert/sites/open-direct-democracy/master/current/app/models/document_element.rb'
require '/home/robert/sites/open-direct-democracy/master/current/app/models/document.rb'
require '/home/robert/sites/open-direct-democracy/master/current/app/models/case.rb'
require '/home/robert/sites/open-direct-democracy/master/current/app/models/case_document.rb'
require '/home/robert/sites/open-direct-democracy/master/current/app/models/case_discussion.rb'
...
CASE_TYPE_LOG = 1
CASE_TYPE_THINGSALYKTUNARTILLAGA = 2
...
class AlthingiCrawler < CaseCrawler
  ...
  def get_thingsalyktun_document(case_id, case_document_id, url)
    puts "GET THYNGSALIKTUNAR DOCUMENT HTML FOR: #{url} case_document: #{case_document_id}"
    html_doc_doc = nil
    retries = 10
    ...
    if html_doc_doc.text.index("Vefskjalið er ekki tilbúið")
      puts "Document not yet ready"
      return nil
    end
  ...
  def get_document(case_id, case_document_id, url)
    puts "GET DOCUMENT HTML FOR: #{url} case_document: #{case_document_id}"
    html_doc_doc = nil
    retries = 10
  ...
  def get_stage_sequence_number(txt, case_type)
    if case_type == CASE_TYPE_LOG
      re1='(\\d+)'  # Integer Number 1
      re2='(\\.)' # Any Single Character 1
      re3='(\\s+)'  # White Space 1
      re4='(umræða)' # Word 1
      ...
      return int1
    elsif case_type == CASE_TYPE_THINGSALYKTUNARTILLAGA
      if txt=="Fyrri umræða"
        return 1
  ...
  def get_case(url, presenter, external_id, external_name, case_type)
   html_doc = nil
   retries = 10
   begin
     Timeout::timeout(120){
   ...
   current_case.info_1 = (html_doc/"h1.FyrirsognStorSv").text.strip
   current_case.info_2 = info_2
   ...
   if case_type == CASE_TYPE_LOG
    current_case.case_type = CaseType.find_by_case_type("Lagafrumvörp")
   elsif case_type == CASE_TYPE_THINGSALYKTUNARTILLAGA
    current_case.case_type = CaseType.find_by_case_type("Þingsályktunatillögur")
   end

   puts "***************************************** New Case *****************************************"
   puts "Case info 1: "+(html_doc/"h1.FyrirsognStorSv").text.strip
   puts "Case info 2: "+info_2
   puts "Case info 3: "+info_3
   current_case.info_3 = info_3
   ...
   if (html_doc/"h2.FyrirsognMidSv").text==""
     ...
     if next_sibling.at("tr[1]/td[1]").text=="Þingskjöl"
       ...
       while next_sibling.at("tr[#{tr_count}]/td[1]")
         ...
         unless oldcd = CaseDocument.find_by_external_link(case_document.external_link)
           ...
           if case_document.external_type.index("frumvarp") or case_document.external_type.index("lög")
             document = get_document(case_document.case_id, case_document.id, case_document.external_link)
             if document
               case_document.save
               document.case_document_id = case_document.id
               document.save
             end
           elsif case_document.external_type.downcase.index("þingsályktun") or case_document.external_type.downcase.index("stjórnartillaga")
             document = get_thingsalyktun_document(case_document.case_id, case_document.id, case_document.external_link)
             if document
               case_document.save
               document.case_document_id = case_document.id
               document.save
     ...
     (html_doc/"h2.FyrirsognMidSv").each do |row|
        # Case stage_sequence
       puts row.text.strip
       stage_sequence_number = get_stage_sequence_number(row.text.strip, case_type).to_i
           ...
           if next_sibling.at("tr[1]/td[1]").text=="Þingskjöl"
             ...
             while next_sibling.at("tr[#{tr_count}]/td[1]")
               ...
               unless oldcd = CaseDocument.find_by_external_link(case_document.external_link)
                 ...
                 puts "Found old case document: " + oldcd.inspect
                 if oldcd.stage_sequence_number != stage_sequence_number
                   puts "FIXING DOC STAGE from: #{oldcd.stage_sequence_number} to: #{stage_sequence_number}"
                   oldcd.stage_sequence_number = stage_sequence_number
                   oldcd.save
                 end
               end
               tr_count+=1
             ...
             while next_sibling.at("tr[#{tr_count}]/td[1]")
               ...
               if next_sibling.search("tr[#{tr_count}]/td[2]/a[@href]")[0]
                 ...
                 case_discussion.transcript_url="http://www.althingi.is"+next_sibling.search("tr[#{tr_count}]/td[2]/a[@href]")[0]['href']
               end
               if next_sibling.search("tr[#{tr_count}]/td[2]/a[@href]")[1]
                 puts "Listen URL: "+next_sibling.search("tr[#{tr_count}]/td[2]/a[@href]")[1]['href']
                 case_discussion.listen_url=next_sibling.search("tr[#{tr_count}]/td[2]/a[@href]")[1]['href']
               end
               puts "Date: "+next_sibling.at("tr[#{tr_count}]/td[1]").text.strip
               case_discussion.meeting_date = DateTime.strptime(next_sibling.at("tr[#{tr_count}]/td[1]").text.strip, "%d.%m.%Y")
               if next_sibling.at("tr[#{tr_count}]/td[3]").text
 ...
 def update_all_cases(case_type)
   if case_type == CASE_TYPE_LOG
   ...
   while next_sibling.at("tr[#{tr_count}]/td[1]")
     ...
     tr_count+=1
     get_case(case_url, case_author, external_case_id, case_name, case_type)
     sleep 1
     puts ""
   end
  end
...
acrawler.update_all_cases(CASE_TYPE_LOG)
acrawler.update_all_cases(CASE_TYPE_THINGSALYKTUNARTILLAGA)

#acrawler.get_case("http://www.althingi.is/dba-bin/ferill.pl?ltg=135&mnr=107", "PRESENTER", "ID", "NAME", CASE_TYPE_THINGSALYKTUNARTILLAGA)
#acrawler.get_case("http://www.althingi.is/dba-bin/ferill.pl?ltg=135&mnr=62", "PRESENTER", "ID", "NAME", CASE_TYPE_THINGSALYKTUNARTILLAGA)
